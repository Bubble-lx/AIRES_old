            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for nt in range(self.num_transitions_per_env):
                    ...
                    if self.seq_expl_len < 1:
                        ...
                        # new_ids = (dones > 0).nonzero(as_tuple=False) # new_ids 的shape是[7, 1]， dones的shape是[512]
                        ...
                        # new_ids = (dones > 0).nonzero(as_tuple=False) # new_ids 的shape是[7, 1]
                ...
                if self.anneal_k: # 参数定义在expl_l5.yaml中
                    anneal_weight = 1. - (step / (num_learning_iterations * self.num_transitions_per_env))
                else:
                    anneal_weight = 1.

                # if self.anneal_k_ratio: # 参数定义在expl_l5.yaml中
                #     anneal_weight_ratio = 1. - (step / (num_learning_iterations * self.num_transitions_per_env))
                # else:
                #     anneal_weight_ratio = 1.
                mean_expl_r = self.storage.compute_expl_r(anneal_weight) # 修改这个函数即可
