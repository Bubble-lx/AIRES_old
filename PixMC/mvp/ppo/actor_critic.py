...
class PixelActorCritic(nn.Module):
    ...
    def act(self, observations, states, step, return_seq_obs=None, eval_mode=False):
        obs_emb, obs_feat = self.obs_enc(observations)  # (B, emb_dim), (B, 384) observations:[512, 3, 224, 224], obs_emb:[512, 128], obs_feat:[512, 384]
        state_emb = self.state_enc(states)  # (B, emb_dim) states:[512, 46]  states_emb:[512, 128]
        joint_emb = torch.cat([obs_emb, state_emb], dim=1)

        actions_mean = self.actor(joint_emb) # [512, 23]
    ...
    @torch.no_grad()
    def obs_enc_functions(self, observations, states, return_seq_obs=None):
