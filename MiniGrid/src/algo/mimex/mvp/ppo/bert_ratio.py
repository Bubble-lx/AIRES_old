...
class BERT_RATIO(nn.Module):
    ...
    def forward_encoder(self, x, seq_act): #突然想起如果将obs*action = obs_next，注意力权重又会如何操作？
        # (B, L, feature_dim) -> (B, L, embed_dim)
        x = self.encoder_embed(x) # 输入为[1024, 5, 39200]输出为[1024, 5, 128]

        a = self.action_embed(seq_act) # 输出为[16384, 5, 64]
        ...
        # 将列表中的张量堆叠成形状为 [4, 1024, 5] 的张量
        mean_attn_all = torch.cat(mean_attn_all, dim=0)
        # 对每个批量的同一序列长度上的4个值进行求和，得到形状为 [1024, 5] 的张量
