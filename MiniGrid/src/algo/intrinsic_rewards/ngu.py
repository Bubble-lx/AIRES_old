...
class NGUModel(IntrinsicRewardBaseModel):
    ...
    def forward(self,
        ...
        curr_cnn_embs = self._get_cnn_embeddings(curr_obs) # 网络1
        next_cnn_embs = self._get_cnn_embeddings(next_obs)

        # print(self._get_cnn_embeddings)


        # Get RNN memories
        if self.use_model_rnn:
            curr_mems = self._get_rnn_embeddings(last_mems, curr_cnn_embs, self.model_rnns) # 网络2
        ...
        pred_act = self.model_mlp.inverse_forward(curr_embs, next_embs) # 网络3  curr_embs:[512, 64]
        ...
        inv_losses = F.cross_entropy(pred_act, curr_act, reduction='none') * (1 - curr_dones) # inv_losses：[16]
        inv_loss = inv_losses.sum() * (1 / n_samples if n_samples > 0 else 0.0) # tensor

        # RND
        rnd_losses, rnd_loss, _ = None, None, None
        if self.ngu_use_rnd:
            curr_rnd_embs, _ = self._get_rnd_embeddings(curr_obs, last_mems)
            # print(self._get_rnd_embeddings)
            # print("--------------------------------")
            tgt_out, prd_out = self.model_mlp.rnd_forward(curr_rnd_embs)
            # print(self.model_mlp)
            # print("--------------------------------")

            rnd_losses = F.mse_loss(prd_out, tgt_out.detach(), reduction='none').mean(-1) * (1 - curr_dones) # rnd_losses:[16]
    ...
    def get_intrinsic_rewards(self, curr_obs, next_obs, last_mems, curr_act, curr_dones, obs_history, stats_logger):
        ...
        # Create IRs
        batch_size = curr_obs.shape[0] # curr_obs:[16, 3, 7, 7]
        int_rews = np.zeros(batch_size, dtype=np.float32)
        for env_id in range(batch_size):
            # Update historical observation embeddings
            curr_emb = ngu_curr_embs[env_id].view(1, -1) # ngu_curr_embs:torch.Size([16, 64]), curr_emb:[1, 64]
            next_emb = ngu_next_embs[env_id].view(1, -1)
            obs_embs = obs_history[env_id] # obs_embs:[33, 64], obs_history中有16个
            new_embs = [curr_emb, next_emb] if obs_embs is None else [obs_embs, next_emb]
            obs_embs = th.cat(new_embs, dim=0)
            obs_history[env_id] = obs_embs # obs_embs:[34, 64]
