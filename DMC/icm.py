...
class ICM(nn.Module):
    ...
    def inverse_dynamics(self, obs, next_obs, actions):
        ...
        loss = self.inv_criterion(a_pred, actions) # loss 是一个值
    ...
    def forward(self, obs, next_obs, actions):
        ...
        icm_loss = self.forward_dynamics(obs_feat, next_obs_feat, actions) # icm_loss：512：[288]
        icm_loss = icm_loss.detach().mean(dim=-1) # icm_loss:[512]
        return icm_loss
