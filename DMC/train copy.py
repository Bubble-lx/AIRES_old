...
import os
os.environ['MKL_SERVICE_FORCE_INTEL'] = '1' # 强制 MKL 使用 Intel-specific optimizations，
# 即使在非 Intel 处理器上也是如此。这可以在某些情况下提高性能，尤其是在使用了 MKL 的数学运算密集型应用程序中。
os.environ['MUJOCO_GL'] = 'egl' # 'egl' 是一种用于 OpenGL ES 和其他图形 API 的接口，它允许不直接依赖于窗口系统进行渲染
...
class Workspace:
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            next_observation = time_step.observation
            if first:
                curr_observation = torch.tensor(curr_observation, dtype=torch.uint8).to(self.device)
                next_observation = torch.tensor(next_observation, dtype=torch.uint8).to(self.device)

                curr_observation = curr_observation.float()
                next_observation = next_observation.float()

                new_embs = [curr_observation.unsqueeze(0), next_observation.unsqueeze(0)]
                obs_embs = torch.cat(new_embs, dim=0)
                # obs_embs通过encoder进行处理
                with torch.no_grad():
                    obs_embs = self.agent.encoder(self.agent.aug(obs_embs)).cpu()

                padding_size = 501 - obs_embs.size(0)
                # zero_padding = torch.zeros(padding_size, obs_embs.size(1), obs_embs.size(2), obs_embs.size(3))
                zero_padding = torch.zeros(padding_size, obs_embs.size(1))
                padded_obs_embs = torch.cat([obs_embs, zero_padding], dim=0)
                obs_embs = padded_obs_embs
                first = False
            else:
                next_observation = torch.tensor(next_observation, dtype=torch.uint8).to(self.device)
                next_observation = next_observation.float()
                ...
                with torch.no_grad():
                    obs_embs_next_obs = self.agent.encoder(self.agent.aug(next_observation)).cpu()

                obs_embs = self.replay_storage._current_episode["obs_history"][-1][:episode_step+1,:]
                # obs_embs = self.replay_storage._current_episode["obs_history"][-1][:episode_step+1,:,:,:]
                obs_embs = torch.cat([obs_embs, obs_embs_next_obs], dim=0)

                padding_size = 501 - obs_embs.size(0)
                if padding_size >= 0:
                    # zero_padding = torch.zeros(padding_size, obs_embs.size(1), obs_embs.size(2), obs_embs.size(3))
                    zero_padding = torch.zeros(padding_size, obs_embs.size(1))
                    obs_embs = torch.cat([obs_embs, zero_padding], dim=0)
                # obs_embs = padded_obs_embs

            self.replay_storage.add_for_ngu(obs_embs)
            ...
            if time_step.last():
                ...
                obs_embs = self.replay_storage._current_episode["obs_history"][-1][:episode_step+1,:]
                # obs_embs = self.replay_storage._current_episode["obs_history"][-1][:episode_step+1,:,:,:]
                # obs_embs = torch.cat([obs_embs, next_observation.unsqueeze(0)], dim=0)
                obs_embs = torch.cat([obs_embs, obs_embs_next_obs], dim=0)
                self.replay_storage.add_for_ngu(obs_embs)

                # 封装&清空
                self.replay_storage.add_my_last()
                pass
            else:
                self.replay_storage.add_my(time_step)
