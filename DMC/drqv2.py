...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
        ...
        if self.use_my_ratio:
            if self.expl_cfg.use_self_encoder:
                ...
                # path = "/home/liuxinn/Download/premier-taco/pretrained_ckpt/encoder.pt"
    ...
    def get_seq_expl_r(self, seq_obs):
        ...
        # if self.firts_time_tmp == 1:
        #     self.initial_weights = [param.clone() for param in encoder.parameters()]
        #     self.firts_time_tmp += 1

        # final_weights = [param for param in encoder.parameters()]
        # for initial, final in zip(self.initial_weights, final_weights):
        #     if torch.equal(initial.data, final.data):
        #         print("Not updated")
        #     else:
        #         print("Updated")
        ...
        seq_obs = self.aug(
            seq_obs.float().view(n, t*c, h, w)).view(n, t, c, h, w) # seq_obs形状不变
        with torch.no_grad():
            if self.expl_cfg.use_actor_feat:
        ...
        # 结合bert_ratio，使用mask以及loss_reward计算bert_loss
        # if self.use_my_ratio:
    ...
    def get_expl_ratio(self, seq_obs, seq_act, seq_next_obs):
        ...
        with torch.no_grad():
            if self.expl_cfg.use_actor_feat: # 默认false
                ...
                seq_next_obs_emb = encoder(seq_next_obs.view(n * t, c, h, w)).view(
                    n, t, -1) # [1024, 5, 39200]
                # seq_emb_self = self.encoder_obs_feat(seq_obs.view(n * t, c, h, w)).view(n, t, -1)
        # 现在流程是：先将seq_obs进行个增强，然后再将其通过encoder编码，形成[1024, 5, 39200]，然后再给bert

        # 计算loss, summed_attn形状为[1024, 5], bert_loss形状为[1024]
        bert_loss, summed_attn = self.bert_ratio(seq_emb.detach(), seq_act, seq_next_obs_emb.detach(), keep_batch=True)
        ...
        ratio = summed_attn  # 应该给的是注意力权重信息
        ...
        # batch_size = 1024
        # seq_length = 5
        # embedding_dim = 39200  # 假设的嵌入维度
        # # 初始化特殊分隔符嵌入为零向量
        # separator_token_embedding = torch.zeros(embedding_dim)
        # # 调整形状以匹配输入序列的维度，并重复以匹配批次大小和序列长度
        # separator_token_embedding = separator_token_embedding.repeat(batch_size, seq_length, 1)

        # # 现在，separator_token_embedding 是一个形状为 [batch_size, seq_length, embedding_dim] 的零张量

        # 拼接状态嵌入、分隔符嵌入和动作嵌入
        # seq_combined = torch.cat((seq_emb, separator_token_embedding, seq_act_embedded), dim=2)
        ...
        if self.expl_cfg.seq_expl_len > 0:
            if self.expl_cfg.baseline == 'icm':
                ...
                if self.use_my_ratio:
                    ...
                    if self.sequencelevel:
                        bert_ratio = torch.mean(bert_ratio.detach(), dim=1)
                ...
                # obs, action, reward, discount, next_obs, seq_obs, seq_act, seq_next_obs, seq_obs_ratio, seq_act_ratio, seq_next_obs_ratio, episode_observation, idx = utils.to_torch(
                #     batch, self.device) # 其中idx表示当前episode

                obs, action, reward, discount, next_obs, seq_obs, seq_act, seq_next_obs, seq_obs_ratio, seq_act_ratio, seq_next_obs_ratio = utils.to_torch(
                    batch, self.device) # 其中idx表示当前episode

                # obs_history = seq_obs[:, -2, :]
                ...
                obs_emb = self.encoder(self.aug(obs.float())) # obs_emb is [512, 39200]
                next_obs_emb = self.encoder(self.aug(next_obs.float()))
                # obs_history_emb = self.encoder(self.aug(obs_history.float()))

                # update inverse dynamics model and get features
                # predict next_obs using forward dynamics model and update model
                # 根据dones把self.episodic_obs_emb_history中的对应数据进行终止
                # episode，idx
                self.episodic_obs_emb_history = [None for _ in range(self.encoder_cfg.batch_size)]
